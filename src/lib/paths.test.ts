...
import { appPath, canonicalForLocale, crossLangHref, pageIdFromPathname, SITE_ORIGIN } from './paths';
...
describe('paths', () => {
  ...
  it('canonicalForLocale', () => {
    ...
    expect(canonicalForLocale('en', 'download')).toBe(`${SITE_ORIGIN}/download`);
    expect(canonicalForLocale('de', 'home')).toBe(`${SITE_ORIGIN}/de/`);
    expect(canonicalForLocale('de', 'download')).toBe(`${SITE_ORIGIN}/de/download`);
  });
