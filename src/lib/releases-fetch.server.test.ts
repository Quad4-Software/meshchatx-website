...
describe('releases-fetch.server', () => {
  ...
  it('parseBundleFile handles bundle shape and legacy array', () => {
    expect(parseBundleFile('{"gitea":[{"tag_name":"v1"}],"github":[]}')).toEqual({
      gitea: [{ tag_name: 'v1' }],
      github: [],
    });
  ...
  it('getReleasesBundle merges disk fallback when live fails', async () => {
    ...
    mockedExists.mockImplementation((p: PathLike) => String(p).includes('releases-bundle.json'));
    mockedRead.mockReturnValue('{"gitea":[{"tag_name":"snap"}],"github":[{"tag_name":"pre"}]}');
