...
function keysForStatus(status: number) {
  if (status === 404) {
    return { titleKey: 'error.title_404' as const, leadKey: 'error.lead_404' as const };
  }
  if (status >= 500) {
    return { titleKey: 'error.title_5xx' as const, leadKey: 'error.lead_5xx' as const };
  }
  return { titleKey: 'error.title_4xx' as const, leadKey: 'error.lead_4xx' as const };
}
