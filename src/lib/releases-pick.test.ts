...
function rel(
  partial: Partial<ReleaseRecord> & { tag_name: string },
): ReleaseRecord {
...
describe("releases-pick", () => {
  it("isLikelyPrereleaseRelease", () => {
    ...
    expect(
      isLikelyPrereleaseRelease(rel({ tag_name: "v1.0.0", prerelease: false })),
    ).toBe(false);
    expect(
      isLikelyPrereleaseRelease(
        rel({ tag_name: "v1.0.0-rc.1", prerelease: false }),
      ),
    ).toBe(true);
    expect(
      isLikelyPrereleaseRelease(rel({ tag_name: "v1.0.0", prerelease: true })),
    ).toBe(true);
  });
  ...
  it("pickPrereleaseRelease prefers github prerelease", () => {
    ...
    expect(pickPrereleaseRelease([giteaStable], [ghPreOld, ghPre])).toEqual(
      ghPre,
    );
  });
