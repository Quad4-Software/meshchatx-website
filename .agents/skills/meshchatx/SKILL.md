...
- Locales: `en` (no prefix), `de`, `es`, `fi`, `fr`, `it`, `nl`, `ru`, `zh`
...
- When `BUNNY_STORAGE_ACCESS_KEY` is set, matching assets prefer Bunny CDN URLs (`https://cdn.quad4.io/...`) with GitHub download URLs as fallback. Storage listings are cached per version.
...
- Flatpak: single-file `.flatpak` download from the selected release, plus CDN remote install commands for `https://cdn.quad4.io/flatpak/` (`.flatpakref` / `.flatpakrepo`). App ID `com.meshchatx.app`. Channel ref files (`meshchatx-beta.flatpakref`, `meshchatx-testing.flatpakref`) exist only after a `beta-*` or `nightly-*`/`testing-*` tag publishes that branch. The OSTree repo carries only the branches that have shipped.
