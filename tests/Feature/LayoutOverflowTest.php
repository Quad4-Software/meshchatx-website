...
{
    public function test_home_main_width_matches_viewport_in_chromium(): void
    {
...
HTML;
        ...
        try {
            ...
            /** @var array{clientWidth:int,scrollWidth:int,bodyScrollWidth:int,mainWidth:?int,shellWidth:?int,bgRight:?int} $metrics */
            ...
            $this->assertLessThanOrEqual(
                ...
                'body must not expand past the viewport',
            );
            $this->assertNotNull($metrics['mainWidth']);
            $this->assertLessThanOrEqual(
                $metrics['clientWidth'] + 1,
                (int) $metrics['mainWidth'],
                'main must not expand past the viewport',
            );
            if ($metrics['bgRight'] !== null) {
                $this->assertLessThanOrEqual(
                    $metrics['clientWidth'] + 1,
                    (int) $metrics['bgRight'],
                    'hero background image must stay inside the viewport',
                );
            }
        } finally {
            @unlink($htmlPath);
